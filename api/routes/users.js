...
router.patch('/:id', ensureCorrectUser, async function(req, res, next) {
  ...
  try {
    ...
    const result = await db.query(
      ...
      [firstname, lastname, username, email, hashedPassword, req.params.id]
    );
    return res.json(result.rows[0]);
  } catch (err) {
    return next(err);
...
router.post('/:id/add-application', ensureCorrectUser, async function(
  req,
  res,
  next
) {
  ...
  try {
    ...
    const result = await db.query(
      ...
      [req.params.id, company_id.rows[0].id, title, location]
    );
    return res.json(result.rows[0]);
  } catch (err) {
    return next(err);
...
router.get('/:id/applications', ensureCorrectUser, async function(
  req,
  res,
  next
) {
